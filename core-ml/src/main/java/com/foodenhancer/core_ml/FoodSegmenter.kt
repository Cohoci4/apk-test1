...
class FoodSegmenter(private val context: Context) {
    ...
    private fun outputToMaskBitmap(output: Array<Array<FloatArray>>): Bitmap {
        val mask = Bitmap.createBitmap(inputSize, inputSize, Bitmap.Config.ARGB_8888)
        for (y in 0 until inputSize) {
            for (x in 0 until inputSize) {
                val value = output[y][x][0]
                val binaryValue = if (value >= threshold) 255 else 0
                mask.setPixel(x, y, Color.argb(255, binaryValue, binaryValue, binaryValue))
            }
        }
        return mask
    }
